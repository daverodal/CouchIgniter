...
<script src="<?=base_url("js/jquery-ui-1.11.0.custom.min.js");?>"></script>
<script src="<?=base_url("js/sync.js");?>"></script>
<script type="text/javascript">
    $(document).ready(function(){
        return;
        var sync = new Sync("<?=site_url("admin/fetchLobby/");?>");
        sync.register("lobbies",function(lobbies){
            ...
            for(var i in lobbies){
                ...
                line += "</span>";
                line += "<a href='<?=site_url('wargame/playAs');?>/"+lobbies[i].id+"'>edit</a> <a href='<?=site_url("admin/deleteGame");?>/"+lobbies[i].id+"/'>delete</a>";
                $("#myGames").append(line);
            }
        });
